...
public class transd extends  ViewableAtomic
{
		...
		if(phaseIs("on"))
		{
			for (int i = 0; i < x.size(); i++)
			{
				if (messageOnPort(x, "ariv", i))
				{
					...
					//System.out.println(val +"is triggered sensor"+c_f.count);
					System.out.println(val + " triggered sensor\n");		//몇번째 알람이 발생했는지 출력한다. 
				...
				if (messageOnPort(x, "solved", i))
				{
					...
					{
						System.out.println("Fire by "+  val + " was extinguished\n");		//불이 꺼졌을 때(solved) 확인 메세지를 출력한다. 
	...
	public double compute_TTA() {
		
	
		return (double) total_ta;
	...
	public void show_state()
	{
		...
		if (arrived != null && solved != null)
		{
			...
			System.out.println("Total TA = " + compute_TTA());
